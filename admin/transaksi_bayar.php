...
<form id="form" method="POST" action="transaksi_bayar.php?id=<?= $data['id_transaksi'] ?>">
    ...
    <label>Total Yang Harus Di Bayar</label>
            <div class="content-inputs"> 
                    <input type="text"  value="<?= $data['total_harga'] ?>" name="total_harga" class="input" required>
            </div>

    <label>Masukan Jumlah Pembayaran</label>
            <div class="content-inputs"> 
                    <input type="number"  value="<?= $edit['telp_member']; ?>" name="total_bayar" id="total_bayar" class="input" required>
